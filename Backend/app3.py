...
# @app.post("/api/departments/{dept_id}/add-class-teacher")
# @require_any_role("hod")
# def add_class_teacher(dept_id: str, payload: AddUserReq = Depends(), section: str = Form(...), current_user=Depends(get_current_user)):
    
#     # Step 1: check HOD owns this department
#     hod_map = db_find_one("department_hods", hod_id=current_user["id"], dept_id=dept_id)
#     if not hod_map:
#         raise HTTPException(403, "Not HOD of this department")

#     # Step 2: create new teacher user
#     user = get_user_row_by_email(payload.email)
#     if user:
#         raise HTTPException(400, "User already exists")
...
#     # Step 3: store class_teacher → department + section mapping
#     db_insert_one("dept_class_teachers", {
#         "id": str(uuid.uuid4()),
#         "dept_id": dept_id,
#         "class_teacher_id": user_id,
...
@app.get("/api/section/{section}/students")
def get_section_students(section: str):
...
@app.get("/api/section/{section}/notes")
def get_section_notes(section: str):
    classes = db_find_many("classes", section=section)
    class_ids = [c["id"] for c in classes]
    ...
    return notes
@app.get("/api/section/{section}/grades")
def get_section_grades(section: str):
    classes = db_find_many("classes", section=section)
    class_ids = [c["id"] for c in classes]

    grades = []
    for gid in class_ids:
        grades += db_find_many("final_marks", class_id=gid)

    return grades

@app.get("/api/section/{section}/grades/{student_id}")
def get_section_student_grade(section: str, student_id: str):
    classes = db_find_many("classes", section=section)
    class_ids = [c["id"] for c in classes]

    marks = []
    for cid in class_ids:
        marks += db_find_many("final_marks", class_id=cid, student_id=student_id)
...
def upload_note(
    ...
    # Check teacher belongs to class 
    cm = db_find_one("class_memberships", class_id=class_id, user_id=current_user["id"])
    if not cm or cm.get("role") != "teacher":
        raise HTTPException(status_code=403, detail="Only subject teacher can upload notes")

    # Create folder 
    bucket_path = UPLOAD_DIR / "notes" / class_id
    bucket_path.mkdir(parents=True, exist_ok=True)
    ...
    relative_path = f"uploads/notes/{class_id}/{filename}"
